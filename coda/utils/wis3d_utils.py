...
def make_wis3d(output_dir="outputs/wis3d", name="debug", time_postfix=False):
    ...
    Make a Wis3D instance. e.g.:
        from coda.utils.wis3d_utils import make_wis3d
...
def convert_motion_as_line_mesh(motion, skeleton_type="smpl22", const_color=None, radius=0.02):
    ...
    for f in range(length):
        vertices_, faces, vertex_colors = create_skeleton_mesh(
            s_points[f], e_points[f], radius=radius, color=m_colors[f]
        )
...
def add_motion_as_lines(
    motion, wis3d, name="joints22", skeleton_type="smpl22", const_color=None, offset=0, radius=0.02
):
    """
    Args:
        motion (tensor): (L, J, 3)
    ...
    vertices, faces, vertex_colors = convert_motion_as_line_mesh(
        motion, skeleton_type=skeleton_type, const_color=const_color, radius=radius
    )
...
def add_lines_as_mesh(wis3d, s_point, e_point, name, radius=0.002, color="red"):
    num_line = s_point.shape[0]
    color_ = get_const_colors(color, partial_shape=(num_line,), alpha=1.0).to(s_point.device)
    color_ = color_[..., :3] * 255
    vertices_, faces, vertex_colors = create_skeleton_mesh(s_point, e_point, radius=radius, color=color_)
    wis3d.add_mesh(vertices_, faces, vertex_colors, name=name)  # Add line as cylinders
    return
