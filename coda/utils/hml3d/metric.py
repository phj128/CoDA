...
def calculate_top_k_with_duplicates(mat, top_k, input_texts=None):
    """
    计算考虑输入重复的top-k准确率

    Args:
        mat: [N, N] 距离矩阵的argsort结果
        top_k: int top-k值
        input_texts: [N] 输入点的文本标识列表

    Returns:
        top_k_mat: [N, top_k] 每个样本在每个k值下是否正确的布尔矩阵
    """
    size = mat.shape[0]
    device = mat.device

    if input_texts is None:
        return calculate_top_k(mat, top_k)

    # 为每个输入点创建布尔向量
    bool_mat = torch.zeros(size, top_k, dtype=torch.bool, device=device)

    for k in range(top_k):
        for i in range(size):
            # 检查输入点i在top-k中是否匹配到正确的输出
            if k < mat.shape[1]:
                # 对于重复的输入文本，只要有一个匹配到正确的输出就算正确
                current_text = input_texts[i]
                same_text_indices = [j for j, t in enumerate(input_texts) if t == current_text]

                correct_for_this_text = False
                # 检查这些相同文本的输入点中是否有任何一个在top-k中匹配到正确的输出
                for input_idx in same_text_indices:
                    if input_idx < mat.shape[0]:
                        # 检查这个输入点在top-k中是否有任何一个匹配到正确的输出
                        top_k_indices = mat[input_idx, : k + 1]  # top-k的索引
                        if input_idx in top_k_indices:  # 如果正确输出在top-k中
                            correct_for_this_text = True
                            break

                bool_mat[i, k] = correct_for_this_text
