...
def xzdir2rotmat(xzdir):
    ...
    """
    x_xzdir = xzdir[..., :2]
    x_xzdir = x_xzdir / x_xzdir.norm(dim=-1, keepdim=True)
    z_xzdir = xzdir[..., 2:4]
    z_xzdir = z_xzdir / z_xzdir.norm(dim=-1, keepdim=True)
...
def get_mat_BtoA_inW(matA, matB):
    """
        return transformation from A to B in the coordinate of W
        Used in IK rotation solving

    Args:
        matA (tensor): [4, 4] world matrix
        matB (tensor): [4, 4] world matrix
    """
    mat_BtoA = get_mat_BtoA(matA, matB)
    rot_BtoA = get_rotation(mat_BtoA)
    aa_BtoA = matrix_to_axis_angle(rot_BtoA)
    aa_BtoA_inW = get_direction_from(aa_BtoA[..., None, :], matA)[..., 0, :]
    rot_BtoA_inW = axis_angle_to_matrix(aa_BtoA_inW)
    if matA.shape[-1] == 4:
        raise NotImplementedError  # Did not check this part
        pos_BtoA_inW = get_position_from_rotmat(get_position(mat_BtoA), matA)
        mat_BtoA_inW = get_TRS(rot_BtoA_inW, pos_BtoA_inW)
    else:
        mat_BtoA_inW = rot_BtoA_inW
    return mat_BtoA_inW


def get_mat_BfromA_inW(matA, matBtoA_inW):
    """
        return world matrix B given matrix A and mat B realtive to A in the coordinate of W
    ...
    """
    if isinstance(matA, torch.Tensor):
        matB = torch.matmul(matBtoA_inW, matA)
    if isinstance(matA, np.ndarray):
        matB = np.matmul(matBtoA_inW, matA)
    matB = normalized_matrix(matB)
    return matB


def get_mat_CtoA_inB_retarget(matCtoA, matBtoA):
    """
        Retargeting
        Change the basis of the coordinate from A to B
        The identity state is different
        Keep the rotation achieves the same target rot, the basis is different.

        # from https://www.bilibili.com/video/BV1GG4y1p7fF?p=4&vd_source=a63694cf86898f9b2b3f4d6d997aac63
        # NOTE: not sure whether this is correct

    Args:
        matCtoA (tensor): [4, 4] C in A coordinate, keeps A identity state
        matBtoA (tensor): [4, 4] B in A coordinate, keeps A identity state
    """
    # matBtoA: points in B coord to points in A coord, do not transform points -> matBtoA * points
    # Now the points in B are the new identity state, we should transform them to A
    # First transform points in B to coordinate A, matAtoB * points
    matAtoB = torch.inverse(matBtoA)  # This is the new identity state
    # Then transform points in A to C pose
    matCtoB = torch.matmul(matCtoA, matAtoB)
    # Here matCtoB is for the new identity state(B)
    return matCtoB


def get_mat_DtoC_retarget(matCtoA, matDtoA, matBtoA_forC, matBtoA_forD):
    """
        Retargeting
        Change the basis of the coordinate from A to B
        The identity state is different, each joint (C and D) both different
        Keep the rotation achieves the same target rot, the basis is different.

    Args:
        matCtoA (tensor): [4, 4] C in A coordinate, keeps A identity state
        matDtoA (tensor): [4, 4] D in A coordinate, keeps A identity state
        matBtoA_forC (tensor): [4, 4] For C joint, B in A coordinate, keeps A identity state
        matBtoA_forD (tensor): [4, 4] For D joint, B in A coordinate, keeps A identity state
    """
    mat_CtoB = get_mat_CtoA_inB_retarget(matCtoA, matBtoA_forC)
    mat_DtoB = get_mat_CtoA_inB_retarget(matDtoA, matBtoA_forD)
    mat_DtoC = get_mat_BtoA(mat_CtoB, mat_DtoB)
    return mat_DtoC
...
def get_relative_position_to(pos, mat):
    ...
    Args:
        pos (_type_): [N, M, 3] or [N, 3] or [N, 3]
        mat (_type_): [N, 4, 4] or [4, 4] or [N, 4, 4]

    Returns:
        _type_: _description_
    """
    pos_shape = pos.shape
    if len(pos_shape) + 1 == len(mat.shape):
        pos = pos[..., None, :]
...
def get_position_from(pos, mat):
    ...
    if isinstance(mat, torch.Tensor):
        rot_pos = torch.matmul(mat[..., :3, :3], pos.transpose(-1, -2)).transpose(-1, -2)
    elif isinstance(mat, np.ndarray):
        rot_pos = np.matmul(mat[..., :3, :3], pos.swapaxes(-1, -2)).swapaxes(-1, -2)
    else:
        raise ValueError
    world_pos = rot_pos + mat[..., None, :3, 3]
...
def get_relative_direction_to(dir, mat):
    ...
    Args:
        dir (_type_): [N, M, 3] or [N, 3] or [N, 3]
        mat (_type_): [N, 4, 4] or [4, 4] or [N, 4, 4]
    ...
    """
    dir_shape = dir.shape
    if len(dir_shape) + 1 == len(mat.shape):
        dir = dir[..., None, :]
    ...
    if isinstance(mat, torch.Tensor):
        rel_dir = torch.matmul(rot_mat_inv, dir.transpose(-1, -2)).transpose(-1, -2)
    elif isinstance(mat, np.ndarray):
        rel_dir = np.matmul(rot_mat_inv, dir.swapaxes(-1, -2)).swapaxes(-1, -2)
    else:
        raise ValueError
...
def get_direction_from(dir, mat):
    ...
    rot_mat = mat[..., :3, :3]
    if isinstance(mat, torch.Tensor):
        world_dir = torch.matmul(rot_mat, dir.transpose(-1, -2)).transpose(-1, -2)
    elif isinstance(mat, np.ndarray):
        world_dir = np.matmul(rot_mat, dir.swapaxes(-1, -2)).swapaxes(-1, -2)
    else:
        raise ValueError

    if len(dir_shape) + 1 == len(mat.shape):
        world_dir = world_dir[..., 0, :]
    return world_dir
...
def find_local_mat(global_mat, parent):
    local_mat = get_mat_BtoA(global_mat[..., parent, :, :], global_mat)
    local_mat[..., 0, :, :] = global_mat[..., 0, :, :]
    return local_mat


def mirror_pos_yzplane(pos):
    pos = pos.clone()
    pos[..., 0] *= -1
    return pos


def mirror_rot_yzplane(rot_aa):
    rot_aa = rot_aa.clone()
    rot_aa[..., 1] *= -1
    rot_aa[..., 2] *= -1
    return rot_aa
