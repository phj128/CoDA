...
from coda.model.diff.utils.motion3d_endecoder import HmlvecArcticEnDecoder, HmlobjvecArcticEnDecoder
...
from coda.utils.hml3d.metric import (
    ...
    calculate_top_k,
    calculate_top_k_with_duplicates,
...
class MetricWholeBodyObjPose(pl.Callback):
    def __init__(self, prefix=""):
        ...
        self.checkpoint_name = "arcticobj"
        self.endecoder_opt = {"_target_": "coda.model.diff.utils.motion3d_endecoder.HmlobjvecArcticEnDecoder"}
        ...
        try:
            ...
            self.ori_text = ListAggregator()
            self._get_t2m_evaluator()
        except Exception as e:
            self.is_fid = False

        self.data_endecoder: HmlobjvecArcticEnDecoder = instantiate(self.endecoder_opt, _recursive_=False)
    ...
    def _get_t2m_evaluator(self):
        ...
        t2m_checkpoint = torch.load(
            os.path.join(opt["checkpoints_dir"], "text_mot_match/model/latest.tar"),
            map_location="cpu",
        )
    ...
    def on_predict_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=0):
        ...
        if "pred_joint_pos" in outputs:
            ...
            pred_angles = outputs["pred_obj_angles"]  # (B, L, 1)
        else:
            ...
            pred_angles = outputs["obj_angles"]
        ...
        articulated_axis = torch.tensor([0, 0, -1], dtype=torch.float32, device=pred_obj_center_mat.device)  # (3, )
        articulated_axis = articulated_axis[None, None].expand(-1, pred_obj_center_mat.shape[1], -1)  # (B, L, 3)
        pred_aa = pred_angles * articulated_axis
        pred_arti_rotmat = matrix.axis_angle_to_matrix(pred_aa)  # (B, L, 3, 3)
        pred_arti_mat = matrix.get_TRS(pred_arti_rotmat, torch.zeros_like(pred_obj_center))  # (B, L, 4, 4)
        pred_arti_mat = matrix.get_mat_BfromA(pred_obj_root_mat, pred_arti_mat)

        gt_angles = batch["angles"]
        gt_aa = gt_angles * articulated_axis
        gt_arti_rotmat = matrix.axis_angle_to_matrix(gt_aa)
        gt_arti_mat = matrix.get_TRS(gt_arti_rotmat, torch.zeros_like(obj_center))
        gt_arti_mat = matrix.get_mat_BfromA(obj_root_mat, gt_arti_mat)

        pred_invpos1 = matrix.get_relative_position_to(pred_pos, pred_obj_center_mat)
        pred_invpos2 = matrix.get_relative_position_to(pred_pos, pred_arti_mat)
        pred_vec = torch.cat([pred_pos.flatten(-2), pred_invpos1.flatten(-2), pred_invpos2.flatten(-2)], dim=-1)
        pred_vec = torch.cat([pred_vec, pred_obj_center, pred_angles], dim=-1)

        gt_invpos1 = matrix.get_relative_position_to(gt_pos, obj_center_mat)
        gt_invpos2 = matrix.get_relative_position_to(gt_pos, gt_arti_mat)
        gt_vec = torch.cat([gt_pos.flatten(-2), gt_invpos1.flatten(-2), gt_invpos2.flatten(-2)], dim=-1)
        gt_vec = torch.cat([gt_vec, obj_center, gt_angles], dim=-1)
    ...
    def on_predict_epoch_end(self, trainer, pl_module):
        ...
        for idx in shuffle_idx:
            all_ori_texts.append(self.ori_text.get_tensor()[idx])
        ...
        for i in range(R_N):
            ...
            group_ori_texts = all_ori_texts[i * self.R_size : (i + 1) * self.R_size]
...
MainStore.store(
    name="metric_wholebodyobjpose",
    node=wholebodyobjpose_node,
    group="callbacks",
    package="callbacks.metric_wholebodyobjpose",
)
