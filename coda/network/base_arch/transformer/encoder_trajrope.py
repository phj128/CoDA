...
from coda.network.base_arch.embeddings.rotary_embedding import ROPE
from coda.network.base_arch.embeddings.cape import cape_embed
from coda.network.base_arch.transformer.encoder_rope import RoPEAttention
from coda.network.base_arch.transformer.encoder import Attention


class TrajRoPEAttention(nn.Module):
    ...
    def forward(self, x, traj, attn_mask=None, key_padding_mask=None):
        # x: (B, L, C)
        # traj: (B, L, 4, 4)
        # attn_mask: (L, L)
        ...
        xq = xq.reshape(B, L, self.num_heads, -1).transpose(1, 2)  # B, N, L, C
        xk = xk.reshape(B, L, self.num_heads, -1).transpose(1, 2)  # B, N, L, C
        xv = xv.reshape(B, L, self.num_heads, -1).transpose(1, 2)  # B, N, L, C

        xq = cape_embed(xq, traj, is_query=True)  # B, N, L, C
        xk = cape_embed(xk, traj, is_query=False)  # B, N, L, C
...
class EncoderTrajRoPEBlock(nn.Module):
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, dropout=0.1, **block_kwargs):
        ...
        self.trajattn = TrajRoPEAttention(hidden_size, num_heads, dropout)
        ...
        self.gate_msa2 = nn.Parameter(torch.zeros(1, 1, hidden_size))
        self.gate_mlp = nn.Parameter(torch.zeros(1, 1, hidden_size))

        # Zero-out adaLN modulation layers
        nn.init.constant_(self.gate_msa, 0)
        nn.init.constant_(self.gate_msa2, 0)
        nn.init.constant_(self.gate_mlp, 0)

    def forward(self, x, traj, attn_mask=None, tgt_key_padding_mask=None):
        x = x + self.gate_msa * self._sa_block(
            self.norm1(x), attn_mask=attn_mask, key_padding_mask=tgt_key_padding_mask
        )
        x = x + self.gate_msa2 * self._traja_block(
            self.norm2(x), traj, attn_mask=attn_mask, key_padding_mask=tgt_key_padding_mask
        )
    ...
    def _traja_block(self, x, traj, attn_mask=None, key_padding_mask=None):
        # x: (B, L, C)
        x = self.trajattn(x, traj, attn_mask=attn_mask, key_padding_mask=key_padding_mask)
        return x
...
class EncoderTrajRoPEBlockDIT(nn.Module):
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, dropout=0.1, **block_kwargs):
        super().__init__()
        self.norm1 = nn.LayerNorm(hidden_size, elementwise_affine=True, eps=1e-6)
        self.attn = RoPEAttention(hidden_size, num_heads, dropout)
        ...
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(),
            nn.Linear(hidden_size, hidden_size * 9, bias=True),
        )

    def forward(self, x, traj, c, attn_mask=None, tgt_key_padding_mask=None):
        shift_msa, scale_msa, gate_msa, shift_traj, scale_traj, gate_traj, shift_mlp, scale_mlp, gate_mlp = (
        ...
        x = x + gate_traj * self._traja_block(
            modulate(self.norm2(x), shift_traj, scale_traj),
            traj,
            attn_mask=attn_mask,
...
class EncoderTrajRoPEBlockDITabsT(nn.Module):
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, dropout=0.1, **block_kwargs):
        super().__init__()
        self.norm1 = nn.LayerNorm(hidden_size, elementwise_affine=True, eps=1e-6)
