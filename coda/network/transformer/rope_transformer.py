...
from coda.network.base_arch.transformer.encoder_rope import EncoderRoPEBlock, EncoderRoPEBlockDIT
...
class NetworkEncoderRoPE(nn.Module):
    def __init__(
        ...
        block_cls = EncoderRoPEBlockDIT if self.is_dit else EncoderRoPEBlock
    ...
    def forward(
        ...
        **kwargs
        ...
        if first_x is not None and hasattr(self, "first_x_embedder"):
            f_to_add.append(self.first_x_embedder(first_x))
...
# Add to MainStore
group_name = "network/diffusion"
MainStore.store(
    name="invtraj_transformer",
    node=builds(NetworkEncoderRoPE, populate_full_signature=True),
    group=group_name,
)
MainStore.store(
    name="bpsinvtraj_transformer",
    node=builds(NetworkEncoderRoPE, objtraj_dim=11, bps_dim=256, populate_full_signature=True),
    group=group_name,
)
MainStore.store(
    name="bpstraj_transformer",
    node=builds(NetworkEncoderRoPE, output_dim=3082, objtraj_dim=17, bps_dim=256, populate_full_signature=True),
    group=group_name,
)
MainStore.store(
    name="bpstrajnorot_transformer",
    node=builds(NetworkEncoderRoPE, output_dim=3082, objtraj_dim=4, bps_dim=256, populate_full_signature=True),
    group=group_name,
)
MainStore.store(
    name="handpose_transformer",
    node=builds(NetworkEncoderRoPE, max_len=300, output_dim=90, objtraj_dim=0, populate_full_signature=True),
    group=group_name,
)
MainStore.store(
    name="bodypose_transformer",
    node=builds(
        NetworkEncoderRoPE,
        max_len=300,
        output_dim=143,
...
MainStore.store(
    name="bodypose_angley_transformer",
    node=builds(
        NetworkEncoderRoPE,
        max_len=300,
        output_dim=140,
